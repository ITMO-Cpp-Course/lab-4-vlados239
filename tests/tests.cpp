...
// ─── FileHandle – чтение и запись ────────────────────────────────────────────

TEST_CASE("Запись и последующее чтение возвращают те же данные", "[FileHandle]")
{
    std::string path = makeTempFile();
    ...
    } // файл закрывается и данные сбрасываются на диск
    {
        ...
        REQUIRE(rh.read() == "Hello, RAII!");
    }

    fs::remove(path);
}

TEST_CASE("Запись в закрытый файл бросает ResourceError", "[FileHandle]")
{
    std::string path = makeTempFile();
    FileHandle fh(path, "w");
    fh.close();
    REQUIRE_THROWS_AS(fh.write("oops"), ResourceError);
    fs::remove(path);
}
